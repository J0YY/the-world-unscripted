...
interface SplitFlapTextProps {
  ...
  scale?: number;
  /**
   * Optional kerning overrides in px applied as extra right margin after a character index.
   * Positive increases spacing, negative pulls the next tile closer.
   */
  kerningRightPx?: Record<number, number>;
}

const CHARSET = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789".split("");

function SplitFlapTextInner({ text, className = "", speed = 50, scale = 1, kerningRightPx }: SplitFlapTextProps) {
  const chars = useMemo(() => text.split(""), [text]);
  const [animationKey, setAnimationKey] = useState(0);
  const [hasInitialized, setHasInitialized] = useState(false);
  const audio = useSplitFlapAudio();

  const handleMouseEnter = useCallback(() => setAnimationKey((prev) => prev + 1), []);
  ...
  return (
    <div
      ...
      style={{ perspective: "1000px", transform: `scale(${scale})`, transformOrigin: "left center" }}
    >
      {chars.map((char, index) => {
        const mr = kerningRightPx?.[index] ?? 0;
        return (
          <div key={index} style={mr ? { marginRight: mr } : undefined}>
            <SplitFlapChar
...
function SplitFlapChar({
  ...
  if (isSpace) {
    return <div style={{ width: "0.3em", fontSize: "clamp(3rem, 10vw, 10rem)" }} />;
  }
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        // Slightly wider tiles so wide glyphs (e.g. W/M) don't get clipped.
        width: "0.74em",
        height: "1.05em",
